...
**Correção de raiz proposta (decisão do dono):** A) pente-fino diário reusando `tiny_fila` + workflow de backfill (API v2, sem token novo); B) ~~"o último pacote do Tiny vence"~~ → **D-50 (dono, 23/09): edição no Tiny edita aqui, apagar no Tiny não apaga aqui — só `obs`/`obs_interna` acompanham o apagar** (janela 60 dias, 3h); C) identidade do cliente por `tiny_id_contato`; ~~D) combinado de processo~~ — descartado pelo dono (a plataforma não depende da equipe de vendas). Demanda **📐 pronta para code** desde 23/09. Demanda: [[SESSAO-29 - Reconciliacao Tiny - Pente-fino e Ultimo Pacote Vence]] (🔶 rascunho).
