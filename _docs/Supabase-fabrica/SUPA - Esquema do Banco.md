...
|---|---|
| `id` | bigint identity PK |
| `tipo` | text (`inclusao_pedido` / `atualizacao_pedido` / `backfill` / `produto_fabrica` / `estoque_fabrica`) |
| `tiny_id` / `numero` | bigint / integer |
| `situacao` | text |
| `recebido_em` | timestamptz default now() |
| `payload` | jsonb — 🆕 21/09/2026 (migration 23): corpo cru quando o formato não é documentado. Nulo nos eventos de pedido |

Tipos novos desde 21/09: **`produto_fabrica`** (cada produto novo/alterado do Tiny da fábrica, gravado por `fn_upsert_produto`) · **`estoque_fabrica`** (webhook "lançamentos de estoque" do Tiny da fábrica, corpo inteiro em `payload`).

## Tabela `produtos` — ✅ aplicada em 2026-09-21 (migration 23)

Catálogo de produtos da Domoby, espelho do **Tiny da FÁBRICA** (FábricaDomoby, CNPJ 27.556.613/0001-66) — **todas as classes**. Escrita só pelo n8n (`domoby-tiny-fabrica-produtos.json`) via `fn_upsert_produto`. RLS ligado sem policy (só service_role). Detalhe: [[N8N - Tiny Fabrica Produtos para Banco]].

| Coluna | Tipo | Observação |
|---|---|---|
| `tiny_id` | bigint **PK** | id interno do produto no Tiny da fábrica |
| `codigo` | text | SKU — **nulo/repetido permitido** (15 fabricados sem SKU; loja reusa SKU em personalizado). Indexado (`produtos_codigo_idx`) |
| `descricao` | text not null default '' | `nome` no Tiny |
| `classe` | text | `classe_produto`: **F** fabricado · **M** matéria-prima · **S** simples · **K** kit · **V** com variação |
| `tipo_variacao` | text | N normal · P pai · V filho |
| `id_produto_pai` | bigint | quando `tipo_variacao = 'V'` |
| `unidade` | text | **já normalizada** na função: un, pc, m, m2, cx, kg, l, par, rolo, chapa, min |
| `estoque_minimo` / `estoque_maximo` | numeric(14,4) | vêm do cadastro do Tiny (48 fabricados com mínimo em 21/09) |
...
`POST {URL}/rest/v1/rpc/fn_upsert_produto` com body `{"p": <retorno.produto cru>}`. SECURITY DEFINER, `search_path=public`, execute só para `service_role`. Idempotente; **só grava quando o `raw` mudou** e, quando grava, registra 1 linha em `eventos` (`tipo = 'produto_fabrica'`, `tiny_id` = id do produto, `situacao`).
...
**Filas reais, tempo de PCP e pausa (SESSAO-22, migration 29 — aplicada em 21/09/2026, D-48):** trigger BEFORE novo **`plt_eventos_resolver_etapa_fila`** (`fn_resolver_etapa_fila` em `plt_privado`): evento de chegada em setor de **produção** sem etapa é completado NA ESCRITA com a etapa `eh_fila` ativa do setor — o fim da coluna "Chegada" nesses quadros (PCP/terminais intactos). `plt_cards` ganhou **`pausado_em`** (projeção de `execucao_pausada`/`execucao_retomada`) e **`liberado_completo_em`** (só card de pedido; recalculada por `fn_recalcular_liberacao` — regra k/n — a cada liberação e `pedido_atualizado`, retroativa; é o fim do tempo em PCP — D-48: o tempo é do PEDIDO — e o filtro de "pedidos abertos" do quadro, com índice parcial `plt_cards_pcp_abertos_idx`). Tipos novos **`execucao_pausada`/`execucao_retomada`** no check (o `validate` saiu da migration 25 — quem valida é sempre a migration mais nova do check, E-19). `fn_validar_execucao` recriada: pausa só por líder do setor do card/admin em execução aberta; retomada por quem executa/líder/admin **passando pela trava do limite** ("finalize a urgência antes"); o limite (iniciar E retomar) **ignora pausados**; referências preenchidas pelo trigger (pausa→iniciar aberto, retomada→pausa). `fn_projetar_posicao` recriada (projeta pausa/retomada/liberação; mover/finalizar/estorno/arquivar zeram a pausa). Helper novo `plt_privado.fn_pausas_execucao` (INVOKER — intervalos pausados recortados a uma janela). **`plt_vw_execucoes` recriada** (drop+create — E-17): colunas novas `pausado_desde`/`pausa_total` e `duracao` DESCONTANDO pausas; `plt_vw_permanencias` fecha a permanência do card de PEDIDO no PCP em `liberado_completo_em`. **6 portas recriadas descontando pausas**: `plt_fn_dash_execucoes/tempos_setor/tempos_pessoa/tempos_item/metas_painel/tendencia_semanas`. `plt_fn_pedidos_kanban` (drop+create) ganhou `entrou_pcp_em`/`liberado_completo_em`; `plt_fn_pedidos_aguardo` (drop+create; o drop-espelho entrou na migration 25 — E-17) ganhou `primeira_pronta_em`/`completo_em` (o relógio do aguardo, insumo do futuro cálculo de tempo de entrega). RPC nova **`plt_fn_definir_limite_execucoes`** (líder do setor/admin; grava na trilha — D-40; +1 WARN esperado nos advisors). `plt_setores.limite_execucoes_por_pessoa` com **default 1** (D-48). Manutenções do mesmo dia (`supabase/manutencao/2026-09-21_*`): limite 1 aplicado aos 9 setores existentes e **1 card vivo** ("Chegada" da FURAÇÃO) migrado por evento origem `api`. Impressão digital da integração antes = depois (`e2109f3a…`, 65 colunas nas 5 tabelas) e contagens idênticas. Aplicação pelo **session pooler IPv4** (A-15): `aplicar-migrations.mjs` agora deriva o pooler sozinho testando a CONEXÃO (o `aws-1` da região resolve DNS mas não conhece o tenant; o certo é o `aws-0-ca-central-1`).

**Dashboards de verdade (SESSAO-16, migration 28 — aplicada em 18/09/2026, D-42):** **8 portas de leitura novas** em `public`, todas gateadas por `plt_privado.fn_setores_dashboard` (D-32) e derivadas só de `plt_eventos`/`plt_cards`/views — nenhuma tabela nova (D-47): `plt_fn_dash_agora` (retrato de agora dos 6 setores de produção — fila = card sem executor projetado, execução = com executor; linha `setor_id null` = total com pessoas sem repetição) · `plt_fn_dash_pcp_dia` (pedidos com unidade por liberar — cancelado normalizado não conta — e unidades liberadas no dia) · `plt_fn_dash_dia` (**concluída = chegou ao terminal final**, decisão do dono 17/09; média das mesmas 4 semanas; danificados do dia; pedidos completos aguardando lançamento) · `plt_fn_dash_producao_hora` · `plt_fn_dash_fim_de_linha` (o destino do dia é o ÚLTIMO terminal da unidade — lançar ESTOQUE→ROTAS não duplica) · `plt_fn_dash_danificados_dia` (estado efetivo = parecer, ou a marcação sem parecer) · `plt_fn_dash_danificados_abertos` · `plt_fn_dash_tendencia_semanas` (média semanal do tempo total útil por unidade concluída; **só responde a quem mede TODOS os setores ativos** — a jornada cruza a fábrica). Dia/hora em **America/Fortaleza**. Impressão digital da integração antes = depois (`15152f89…`, 114 colunas). Advisors: **+8 WARN esperados** (DEFINER executável por authenticated — endpoints de propósito, padrão E-11). Front: as 4 telas-filhas do pai Dashboards; `plt_visualizacoes.configuracao` ganhou o formato `{tela, periodoDias, setorId, tempo}` — o formato antigo (widgets, S10) é traduzido por leitura no front, nenhum dado migrado.

**Meu Painel 2.0 (SESSAO-23, migration 33 — `20260924120000_plt_meu_painel_2.sql`, aplicada em 23/09/2026 com permissão total do dono na conversa, D-51):** `plt_tarefas` ganhou **`tarefa_mae_id`** (subtarefa na MESMA tabela; trigger `fn_validar_tarefa` em `plt_privado` trava o 3º nível, a troca de mãe e herda setor/privacidade/responsável da mãe), **`origem`** (`pessoa`/`sistema` — check novo `plt_tarefas_origem_ck`), **`evento_referencia_id`** (a marcação de qualidade que abriu a pendência; índice único parcial `plt_tarefas_sistema_ref_uq`) e **`privada`** (tarefa pessoal invisível a líder/admin; só quando criador = responsável; reatribuir torna pública sozinha; trigger `fn_propagar_privacidade_tarefa` espelha nas filhas/netas). **RLS de `plt_tarefas` recriada**: dono/criador sempre; os demais só o que não é privado; INSERT do navegador só `origem='pessoa'`; tarefa do sistema não se edita à mão (flag de sessão `plt.tarefa_sistema`, só a maquinaria usa). Trigger AFTER novo **`plt_eventos_tarefa_sistema`** (`fn_tarefa_sistema_qualidade`): chegada com marcação em setor de **produção** cria a tarefa do Sistema no setor recebedor + notificação `tarefa_sistema` aos membros; o parecer (ou o card seguir adiante/ser arquivado) conclui a tarefa sozinho; retroativo aplicado (2 pendências abertas viraram tarefa). `plt_usuarios` ganhou **`fila_prioridade`** (jsonb, ordem da fila do Meu Painel — `grant update` da coluna ao authenticated; reordenar gera log `fila_prioridade_reordenada`, `fn_logar_usuario` recriada). `plt_cards` ganhou **`delegado_em`** (projeção do evento de delegação — `fn_projetar_posicao` recriada; retroativo aplicado). **3 RPCs pessoais novas** em `public` (endpoints de propósito — +3 WARN esperados nos advisors): `plt_fn_meu_tempo_dias`, `plt_fn_meu_tempo_tarefas` (paginada), `plt_fn_meu_desempenho` — todas devolvem SÓ o dado de quem chama (o tempo de afazeres pessoais não sai nem para admin; provado no harness com `set role authenticated` — A-21 — e por ensaio A-11 no banco real). Nenhuma tabela da integração tocada (impressão digital idêntica na aplicação).
...
**Segurança herdada (SESSAO-21 — migration 32, `20260923120000_plt_seguranca_herdada.sql`, aplicada em 23/09/2026 com aprovação do dono):** `fn_pedido_por_numero_nf(integer)` perdeu o EXECUTE de `public`/`anon`/`authenticated` (fica `service_role`; quem usa de verdade é `plt_privado.fn_vincular_conta_receber`, SECURITY DEFINER — 0 chamadas pela API em 24h, nenhum workflow do n8n a chama); `fn_backfill_conta_mapear(jsonb)` e `fn_vig_touch()` ganharam `search_path = public, pg_temp`. Impressão digital da integração antes = depois (`e2109f3a…`, 65 colunas); sonda anônima → 401; advisors: os dois apontamentos sumiram. `vig_conhecimento_vendas` sem policy segue **intencional** (Atendimento, só chave de serviço). ⚠️ Esses objetos (e `fn_vincular_conta_receber`, `vig_conhecimento_vendas`) **não têm espelho** no `supabase-fabrica-schema.sql` nem em migration de criação — foram criados direto no banco por outras frentes (classe do E-27); a migration 32 só os toca se existirem.
