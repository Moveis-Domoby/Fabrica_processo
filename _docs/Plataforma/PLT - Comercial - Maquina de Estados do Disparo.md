...
> [!success] ✅ 23/09/2026 — trava ABERTA (camada 1 = `true`), a pedido do dono ([PR #6](https://github.com/Moveis-Domoby/Fabrica_processo/pull/6))
> Depois de o dono trocar a URL do webhook no DataCrazy. Abrir a chave **não envia nada sozinho**: envio só por clique ("Enviar" no membro ou "Iniciar fila", com confirmação), e o cron só envia para lista `disparando` — no momento, nenhuma. Emergência: voltar a `false` trava tudo de novo. O texto abaixo é o estado de 22/09.
