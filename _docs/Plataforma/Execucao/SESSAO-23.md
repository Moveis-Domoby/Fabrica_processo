...
## Segunda rodada de ajustes do dono (23/09 — respostas 1 e 2 + prints)

- **AJUSTE 2 destravado:** o dono confirmou que 13107 e 13196 eram TESTE → manutenção `2026-09-24_concluir_arquivar_pedidos_teste.sql` (movimentação → ESTOQUE + arquivamento, tudo evento origem `api`; 2 avisos de chegada ao ESTOQUE gerados, esperado — D-25). **Achado novo na conferência:** pedido **13257 "Entregue"** no Tiny com 2 unidades REAIS em MONTAGEM desde 23/09 — não é teste; está 100% liberado (fora do quadro de qualquer jeito), reportado ao dono ("Entregue" no Tiny ≠ entregue de fato; a SESSAO-29 é quem reconcilia).
- **Migration 35** (`20260924180000`): `plt_tarefas.tempo_acumulado` + RPC `plt_fn_tarefa_pausar` (pausar GUARDA o tempo; retomar = iniciar) · 3 portas pessoais somando acumulado + segmento aberto · **`plt_fn_cards_pedido_pcp`** (quadro do PCP sem encerrados no Tiny, filtro na consulta com `fn_situacao_normalizada`; gate admin/entrada; paginada com total). Aplicada em 24/09; **quadro do PCP caiu de 161 para 37 pedidos**.
- **E-41 na aplicação:** o backfill do `tempo_acumulado` quebrou SÓ em produção (tarefas do Sistema já existiam; o trigger de imutabilidade recusou o UPDATE sem a flag) — corrigido com a flag `plt.tarefa_sistema` no DO block; o aplicador é atômico, nada parcial ficou. Também: número de pedido de teste 999998 já era usado pelo cenário da S09 no harness → 999993.
- **Front:** botões **▶/⏸/✓ na Fila de prioridade** (pausar/finalizar só aparecem com o tempo rodando); "Em execução agora" saiu do grid e virou a **`BolhaExecucao`** global (bolinha amarela meio transparente, badge, painel com Pausar/Concluir/Ver detalhes → preview; execução de produção linka o quadro); ModalTarefa trocou "Parar (descarta)" por **Pausar (guarda)** + "Retomar"; `buscarCardsPedidoPcp` virou a RPC nova.
- **Validação ao vivo (dono logado):** iniciar pela fila → linha vira ⏸/✓ com o total; bolha aparece com badge 2; pausar pela bolha → linha volta ao ▶ e o banco mostra **39s acumulados**; painel do PCP com 37 abertos (prova por papel simulado). tsc/lint/vitest/build/test:banco 2× verdes; integração intacta na aplicação.
