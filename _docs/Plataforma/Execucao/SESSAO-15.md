...
- [15/09] **Reteste pedido pelo dono ("Realize o teste").** Automáticos: `test:banco` 2 rodadas TUDO VERDE · tsc · lint · Vitest 40/40 · build. Produção (só leitura): impressão digital da integração `15152f89…` inalterada (pedidos 3.857→5.302 com o backfill), 21 tabelas plt_, 42 policies, 2 gatilhos com a guarda `translate`, as 10 portas da S15 presentes, Edge `geocodificar` v1 ativa. **5 eventos `pedido_cancelado` desde 08/09** — a correção da situação normalizada funciona em produção. PCP voltou a 130 cards vivos: 73 cards novos, TODOS de webhook com pedido aberto na chegada (blindagem íntegra); 38 já foram entregues pelo processo antigo (ClickUp→Tiny) — acúmulo esperado enquanto a equipe não usa a plataforma. Sessão do navegador expirou → teste ao vivo aguarda o dono entrar.
